...
public class DataAccumulator {
    ...
    public EnergyReading addEvent(SensorEvent sensorEvent) {
        ...
        if ((e.timestamp - events.getFirst().timestamp) < TIME_INTERVAL) { // Interval has not passed
            ...
            // TODO: Implement average mean square calculation for energy a
            float accumlatorX=0;
            float accumlatorY=0;
            float accumlatorZ=0;

            CustomSensorEvent prev=null;
            for (CustomSensorEvent event:events){
                if(prev!=null){
                    accumlatorX+= Math.pow((event.values[0]-prev.values[0]),2);
                    accumlatorY+= Math.pow((event.values[1]-prev.values[1]),2);
                    accumlatorZ+= Math.pow((event.values[2]-prev.values[2]),2);
                }
                prev=event;
            }

            events=new LinkedList<>();
            return new EnergyReading((accumlatorX+accumlatorY+accumlatorZ)/3,sensorEvent.timestamp);
